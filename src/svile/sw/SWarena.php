...
final class SWarena
{
	...
	private $slot;
		/** @var string */
	private $world;//Seconds to wait before the game starts
	/** @var int */
	private $countdown = 60;//Max seconds after the countdown, if go over this, the game will finish
	/** @var int */
	private $maxtime = 600;//This is used to check "fake void" to avoid fall (stunck in air) bug
	/** @var array */
	...
	public function __construct(SWmain $plugin, $SWname = 'sw', $slot = 0, $world = 'world', $countdown = 60, $maxtime = 300, $void = 0)
	{
		...
		$this->void = $void;
		if (!$this->reload()) {
			$this->pg->getLogger()->info(TextFormat::RED . 'An error occurred while reloading the arena: ' . TextFormat::WHITE . $this->SWname);
	...
	private function reload(): bool
	{
		...
		if ($this->pg->getServer()->getWorldManager()->isWorldLoaded($this->world)) {
			if ($this->pg->getServer()->getWorldManager()->getWorldByName($this->world)->getAutoSave() || $this->pg->config['world.reset.from.tar']) {
	...
	{
		if ($this->GAME_STATE == 0 && count($this->players) < ($this->pg->config['needed.players.to.run.countdown'] + 0))
			return;
		$this->time++;

		//START and STOP
		if ($this->GAME_STATE == 0 && $this->pg->config['start.when.full'] && $this->slot <= count($this->players)) {
			$this->start();
			return;
		}
		...
		//Chest refill
		if ($this->GAME_STATE > 0 && $this->pg->config['chest.refill'] && ($this->time % $this->pg->config['chest.refill.rate']) == 0) {
			$this->refillChests();
		...
		if ($this->GAME_STATE == 2) {
			if ($this->time <= $this->pg->config['no.pvp.countdown'])
				foreach ($this->pg->getServer()->getWorldManager()->getWorldByName($this->world)->getPlayers() as $p)
					$p->sendPopup(str_replace('{COUNT}', $this->pg->config['no.pvp.countdown'] - $this->time + 1, $this->pg->lang['no.pvp.countdown']));
			else
				$this->GAME_STATE = 1;
			return;
		}
		...
		if ($this->GAME_STATE == 0) {
			foreach ($this->pg->getServer()->getWorldManager()->getWorldByName($this->world)->getPlayers() as $p) {
				...
				if (($this->countdown - $this->time) <= 10) {
					foreach ((new ClickSound(1.0))->encode($p->getPosition()->asVector3()) as $sound) {
	...
	{
		if ($this->pg->config['chest.refill'])
			$this->refillChests();
		foreach ($this->players as $name => $spawn) {
			if (($p = $this->pg->getServer()->getPlayerByPrefix($name)) instanceof Player) {
				$p->setMaxHealth($this->pg->config['join.max.health']);
				$p->setMaxHealth($p->getMaxHealth());
				if ($p->getAttributeMap() != null) {//just to be really sure
					if (($health = $this->pg->config['join.health']) > $p->getMaxHealth() || $health < 1)
						$health = $p->getMaxHealth();
					$p->setHealth($health);
					$p->getHungerManager()->setFood(20);
				}
				$p->sendMessage($this->pg->lang['game.start']);
				if ($p->getWorld()->getBlock($p->getPosition()->floor()->subtract(0, 2, 0))->getId() == 20)
					$p->getWorld()->setBlock($p->getPosition()->floor()->subtract(0, 2, 0), VanillaBlocks::AIR(), true, false);
				if ($p->getWorld()->getBlock($p->getPosition()->floor()->subtract(0, 1, 0))->getId() == 20)
					$p->getWorld()->setBlock($p->getPosition()->floor()->subtract(0, 1, 0), VanillaBlocks::AIR(), true, false);
			}
		}
		$this->time = 0;
		$this->GAME_STATE = 2;
	...
	private function refillChests()
	{
		...
		foreach ($this->pg->getServer()->getWorldManager()->getWorldByName($this->world)->getLoadedChunks() as $chunks) {
		...
		foreach ($this->players as $name => $spawn) {
			if (($p = $this->pg->getServer()->getPlayerByPrefix($name)) instanceof Player) {
				...
				if (!$force) {
					...
					//Economy reward
					if ($this->pg->config['reward.winning.players'] && is_numeric($this->pg->config['reward.value']) && is_int($this->pg->config['reward.value'])) {
						BedrockEconomyAPI::getInstance()->addToPlayerBalance($p->getName(), (int)$this->pg->configs['reward.value']);
						$p->sendMessage(str_replace(['{MONEY}', '{VALUE}'], [BedrockEconomyAPI::getInstance()->getPlayerBalance($p->getName())->getResult(), $this->pg->config['reward.value']], $this->pg->lang['winner.reward.msg']));
					}
					//Reward command
					$command = trim($this->pg->config['reward.command']);
					if (strlen($command) > 1 && $command[0] == '/') {
						$this->pg->getServer()->dispatchCommand(new ConsoleCommandSender($this->pg->getServer(), $this->pg->getServer()->getLanguage()), str_replace('{PLAYER}', $p->getName(), substr($command, 1)));
					}
				}
			}
		}
	...
	public function closePlayer(Player $p, $left = false, $spectate = false)
	{
		if ($this->quit($p->getName(), $left, $spectate)) {
			...
			if (!$spectate) {
				...
				$idmeta = explode(':', $this->pg->config['spectator.quit.item']);
...
//                $p->getInventory()->sendContents($p->getViewers());
	...
	public function join(Player $player, $msg = true)
	{
		...
		if($this->inArena($player) || isset($this->players[$player->getName()])){
			return false;
		}
		...
		//Sound//getWorldManager()->getWorldByName
		foreach ((new EndermanTeleportSound())->encode($player->getPosition()->asVector3()) as $sound) {
			$player->getNetworkSession()->sendDataPacket($sound);
		}

		//Removes player things
		$player->setGamemode(GameMode::SURVIVAL());
		if ($this->pg->config['clear.inventory.on.arena.join'])
			$player->getInventory()->clearAll();
		if ($this->pg->config['clear.effects.on.arena.join'])
			$player->getEffects()->clear();
		$player->setMaxHealth($this->pg->config['join.max.health']);
		$player->setMaxHealth($player->getMaxHealth());
		if ($player->getAttributeMap() != null) {//just to be really sure
			if (($health = $this->pg->config['join.health']) > $player->getMaxHealth() || $health < 1)
